...
export default function GhanaCardScanner(){
    ...
    const autoCaptureRef = useRef(false);       // Flag to control auto-capture feature
    ...
    // Submit and Auto-Capture Controls
    const [submitEnabled, setSubmitEnabled] = useState(false);        // Enables or disables the submit button
    const [autoCapture, setAutoCapture] = useState(false);            // Toggle auto-capture mode
    ...
    const stopCamera = () => {
        ...
        //Stop detection loopoka
        if (animationFrameRef.current) {
            cancelAnimationFrame(animationFrameRef.current);
            
            // Reset the animationFrameRef to signal that the loop is stopped
    ...
    //Function to toggle auto-capture
    const toggleAutoCapture = () => {
        autoCaptureRef.current = !autoCaptureRef.current;
        setAutoCapture(autoCaptureRef.current);

        if(autoCaptureRef.current){
            setStatus('Auto-capture enabled. Hold a Ghana Card steady in the frame.');
    ...
    const startDetectionLoop = () => {
        if (!videoRef.current || !canvasRef.current || !cameraRef.current || !modelRef.current) {
        ...
        //Start detection if no frsme is currently being processed
    ...
    //Capture card manually
    const manualCapture = () => {
        // Check if detection system is ready
        if (!detectionRef?.current) {
            setStatus('Error: Detection system not ready.');
            return;
        }

        // Check if there are any detections to capture
        const detections = detectionRef.current;
        if (detections.length > 0) {
            const bestDetection = detections[0]; 
            if (bestDetection.confidence > 0.75) { 
                captureCard(bestDetection, videoRef, captureRef, setStatus, setCaptureMessage, setSubmitEnabled);

            } else {
                setStatus('Detection confidence too low. Please position the card better.');
            }
        } else {
            setStatus('No Ghana Card detected. Please position the card in the frame.');
        }
    };
    ...
    const checkForAutoCapture = (detections) => {
        ...
        if(autoCaptureRef.current){
            ...
            if (detection.confidence > confidenceThreshold && detection.alignmentScore > 0.5) {
              setConsecutiveDetections(prev => {
                const next = prev + 1;
                setStatus(`Card detected - Holding steady: ${next}/${minConsecutiveDetections}`);
                if (next >= minConsecutiveDetections) {
                    captureCard(detection, videoRef, captureRef, setStatus, setCaptureMessage, setSubmitEnabled);
                  setStatus('Card auto-captured successfully')
                  return 0;
                }
                return next;
              });
              ...
              setStatus('Auto-capture active: Position card properly in the frame');
            }
        }
    };
    ...
    useEffect(() => {
        ...
        capturedCtx.fillText(text, capturedCanvas.width / 2, capturedCanvas.height / 2);
    }, [])
    ...
    const sendToBackend = () => {
        ...
        captureRef.current.toBlob(async (blob) => {
            ...
            try {
                ...
                setStatus(`Verification successful! Card is ${data.valid ? 'valid' : 'invalid'}`);
                setCaptureMessage(`Verification result: ${data.valid ? 'Valid' : 'Invalid'} Ghana Card`);
    ...
    return (
        <div className="container">
            ...
            <div className="status-container">
                ...
                <div className="model-status" id='model-status'>{modelStatus}</div>
            </div>
            ...
            <div className="camera-container">
                ...
                <div className="controls">
                    <button className="btn" onClick={handleCameraAction}>{cameraStatus ? 'Stop Camera' : 'Start Camera'}</button>
                    <button className="btn" disabled={!cameraStatus} onClick={manualCapture}>Manual Capture</button>
                    <button 
                        className="btn" 
            ...
            <div className="result-container">
                ...
                <div className="capture-info">
                    ...
                    <button className='btn send-btn' disabled={!submitEnabled} onClick={sendToBackend}>Submit</button>
                </div>
            </div>
            ...
            <div className="instructions">
                ...
                <ol>
                    ...
                    <li>Postion your Ghana Card within the uide frame</li>
